import {
  type ElementUpdate,
  mutateElement,
} from "@excalidraw/excalidraw/element/mutateElement";
import type {
  ElementsMap,
  ExcalidrawElement,
...
export const cleanupPointBoundElements = (
  ...
  elements
    .filter((element) => isEuclidLineElement(element))
    .forEach((element) => {
      if (isEuclidLineElement(element)) {
        euclidLineImplementation.mutateElement(
          element as Mutable<EuclidLineElement>,
...
export const getListOfMutations = (
  ...
  for (const elementId of sortedElements) {
    ...
    if (isEuclidCircleElement(element)) {
      mutation = updatePositionOfCirlce(element, elementsMap, mutationsMap);
    } else if (isEuclidLineElement(element)) {
        ...
        return {
          customData: {
            ...
            origin: {
              ...element.customData.origin,
              x:
                boundElement.customData.points[0].x * position +
                boundElement.customData.points[1].x * (1 - position),
              y:
                boundElement.customData.points[0].y * position +
                boundElement.customData.points[1].y * (1 - position),
            },
          },
        };
